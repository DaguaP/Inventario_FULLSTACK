...
class InterfaceRepositorio(Generic[T]):
    ...
    """####obtener un atributo#####
    def getCantidadProducto(self, id, cantidadProducto):
        laColeccion = self.baseDatos[self.coleccion]
        x = laColeccion.find({id: "_id"}, {cantidadProducto: "_cantidadProducto"})
        return x
    """
    def getObtenerCantidadProducto(self, id):
        laColeccion = self.baseDatos[self.coleccion]
        producto = laColeccion.find_one({"_id": ObjectId(id)})

        if producto and "cantidadProducto" in producto:
            cantidadProducto = producto["cantidadProducto"]
            return cantidadProducto
        return 0


    def getObtenerValorCosto(self, id):
        laColeccion = self.baseDatos[self.coleccion]
        producto = laColeccion.find_one({"_id": ObjectId(id)})

        if producto and "valorCosto" in producto:
            valorCosto = producto["valorCosto"]
            return valorCosto
        return 0


    def getObtenervalorVenta(self, id):
        laColeccion = self.baseDatos[self.coleccion]
        producto = laColeccion.find_one({"_id": ObjectId(id)})

        if producto and "valorVenta" in producto:
